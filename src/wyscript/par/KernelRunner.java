...
//	List<CUdeviceptr> devicePointers = new ArrayList<CUdeviceptr>();
	 ...
	 *
	 * @requires All the types within the type list match up correctly with the Cuda kernel
	 * @ensures The loop runs on the kernel and the values of the computation placed on frame
	 */
	public void run(HashMap<String,Object> frame) {
		...
		for (int i = 0; i < symbolTypes.size();i++) {
			...
			if (expectingLengthArg) {
				//String name = params.get(i); //don't actually need to get name
				...
				if (symType instanceof Type.Int) {
					...
					InternalFailure.internalFailure("Was expecting length " +
							"argument at parameter "+i, writer.getPtxFile().getPath(), type);
				}
				...
				if (((Type.List)type).getElement() instanceof Type.Int) {
					...
					//copy values over, ensuring that they are unwrapped form Integer type.
					for (int j = 0 ; j < expectedLength ; j++) array[j] = listObject.get(j);
					CUdeviceptr dpointer = generatePointer(expectedLength, array);
					kernelParameters[i+1] = Pointer.to(dpointer);
					//TODO I believe the value of the list (on the frame) is ArrayList<Object>
				}
			}//end processing list type
		}//end of the loop over symbol types
		//verify and check twice all the assumptions made up until this point!
		...
		cuCtxSynchronize();
		//now copy all parameters back!
	}

	/**
	 * Allocates device memory for the integer array and returns a pointer to it.
	...
	private CUdeviceptr generatePointer(int expectedLength, int[] intArray) {
		...
		cuMemcpyHtoD(dpointer, Pointer.to(intArray), expectedLength*Sizeof.INT);
		return dpointer;
	}
